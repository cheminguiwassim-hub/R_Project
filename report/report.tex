...
\caption{Evolution of the distribution of sample means for different underlying distributions (Exponential, Uniform, Bernoulli) and increasing sample sizes. For small samples, the distributions retain the shape of the original data, while for larger samples they become more symmetric and bell-shaped. This illustrates the convergence toward a normal distribution predicted by the Central Limit Theorem.}
\end{figure}
...
\caption{Q-Q plot comparing the quantiles of the sample means to those of a theoretical normal distribution. The central points align approximately along the diagonal, indicating partial normality, while deviations in the tails reveal that convergence is not yet perfect. This suggests that the sample size, although large, is not sufficient for complete normal approximation.}
\end{figure}
...
\caption{Convergence of the cumulative sample mean toward the theoretical expected value (approximately 0.5 for a uniform distribution). The fluctuations are large at the beginning due to randomness but decrease as the number of observations increases, illustrating the stabilizing effect described by the Law of Large Numbers.}
\end{figure}
